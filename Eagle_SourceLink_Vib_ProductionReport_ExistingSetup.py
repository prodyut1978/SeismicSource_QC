...
def VibProductionReportExistingSetup():
    Pickle_in      = open("C:\VibRestrictedFolder\VibProductionModule\VibProfileParameter","rb")
    pickle_dict    = (pickle.load(Pickle_in))
    pickle_dict    = list(pickle_dict.values())

    pickle_dict_VibNo = pickle_dict[1:len(pickle_dict):2]
    pickle_dict_VibID = pickle_dict[0:len(pickle_dict):2]
    TotalNumberVibes  = len(pickle_dict_VibID) + 1

    layout = []
    for i in range(1,TotalNumberVibes):    
        layout += [sg.Text(f'{i}. ' "Enter Vib" f'{i}. ' "PSS Profile ID :" , size=(25,1)),  sg.In(pickle_dict_VibID[i-1]),
                   sg.Text(f'      {i}. ' "Enter Vib" f'{i}. ' "Unit No :" , size=(22,1)),  sg.In(pickle_dict_VibNo[i-1])],
        ...
        if (Empty_Check_Profile.values.any()==True) | (Empty_Check_VibNum.values.any()==True) | (Empty_Check_VibPlot.values.any()==True) | (Vib_NumberPlot['Vib_Number'].duplicated().values.any() == True) | (Number_of_Vib['Unit_ID'].duplicated().values.any() == True):
            ...
            if Length_fileList >0:                
                ...
                for i in range(len(VibProfileIDList)):
                    ...
                    plt.title(("Vib Profile ID: " + str(VibProfileID) + '      ' + "Vib Number: " + str(VibNumber)),size=12, verticalalignment='bottom')
