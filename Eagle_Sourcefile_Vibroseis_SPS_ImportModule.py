...
def SourceFile_Vibroseis_ImportModule():
    window = Tk()
    window.title ("Eagle Vibroseis Source File (SPS) Import Wizard")
    ...
    def importSourceSPS():
        ...
        SourcefileList = askopenfilenames(initialdir = "/", title = "Import vibroseis Source Files" , filetypes=[('CSV File', '*.csv'), ('Excel File', ('*.xls', '*.xlsx'))])
        ...
        if Length_fileList >0:        
            if SourcefileList:
                ...
                tkinter.messagebox.showinfo("Import Vib Source File Message","Source File Imported to Database Successfully")
        else:
            tkinter.messagebox.showinfo("Import Vib Source File Error Message","Please Select Vib Source File")
    ...
    def iExit():
        iExit= tkinter.messagebox.askyesno("Eagle Vib Source File Import Wizard", "Confirm if you want to exit")
        if iExit >0:
            window.destroy()
            return
    ...
    ##### Labeling
    L1 = Label(window, text = " Source File Vibroseis:", font=("arial", 10,'bold'),bg = "green").place(x=2,y=6)
    L2 = Label(window, text = "Total Entries", font=("arial", 10,'bold'),bg = "yellow").place(x=460,y=7)

    btnImportSPSFile = Button(window, text="Import Vibroseis Source File", font=('aerial', 9, 'bold'), height =1, width=23, bd=4, command = importSourceSPS)
