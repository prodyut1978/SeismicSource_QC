...
def GenerateHawkTBMissingShotReport():
    ...
    cur=conn.cursor()
    Complete_df  = pd.read_sql_query("SELECT * FROM Eagle_HAWK_OBLog_MASTER ORDER BY `ShotID` ASC ;", conn)
    ...
    if (len(MasterSPS_DF_Merge) >0) & (len(MasterTB_DF) >0):
        ...
        MasterTB_DF  = MasterTB_DF.loc[:,['ShotID','SourceLine','SourceStation', 'MasterSystemFieldRecordID', 'Comment']]
        ...
        if event is None or event == 'Cancel':
            ...
            if len(filename) >0:
                QC_Missing_Rep   = get_QC_Missing_Rep_datetime()
                outfile_QC_Missing_Rep = filename + QC_Missing_Rep 
        ...
        tkinter.messagebox.showinfo("Error In Generating Missing Shot Report","Please Check Imported Hawk TB Log or Imported SPS File to Generate Missing Shot Report Correctly")
