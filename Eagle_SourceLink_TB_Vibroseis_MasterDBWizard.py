...
def SourceLink_TB_SubmitToMasterDB():
    Default_Date_today   = datetime.date.today()
    window = Tk()
    window.title ("Eagle SourceLink Vibroseis Timebreak Import Wizard")
    window.geometry("1250x780+10+0")
    window.config(bg="cadet blue")
    window.resizable(0, 0)
    TableMargin = Frame(window, bd = 2, padx= 1, pady= 30, relief = RIDGE)
    ...
    tree.heading("#12", text="Process", anchor=W)
    tree.heading("#13", text="SweepNumber", anchor=W)
    ...
    tree.column('#13', stretch=NO, minwidth=0, width=110)
    style = ttk.Style()
    ...
    def Update_Vib_MasterDB():
        if(len(txtShotNumber.get())!=0):
            conn = sqlite3.connect("SourceLink_Log.db")
            cur = conn.cursor()
            for selected_item in tree.selection():
                cur.execute("DELETE FROM Eagle_SOURCELINKTBMASTER WHERE FileNum =? AND SourceLine = ? AND SourceStation = ?", (tree.set(selected_item, '#3'), tree.set(selected_item, '#5'), tree.set(selected_item, '#6'),))
                conn.commit()
        ...
        if(len(txtShotNumber.get())!=0):
            try:
                Eagle_SourceLink_Vibroseis_Log_BackEnd.addInvRec(txtTriggerIndex.get(), txtProfileID.get(), txtShotNumber.get(), txtEpNumber.get(), txtShotLine.get(), txtShotStation.get(),
                                                           txtShotUtcDateTime.get(), txtLatitude.get(), txtLongitude.get(), txtShotStatus.get(),
                                                           txtComment.get(), txtProcess.get(), txtSweepNumber.get())
    ...
    def Add_Vib_MasterDB():
        if(len(txtShotNumber.get())!=0) & (len(txtShotLine.get())!=0) & (len(txtShotStation.get())!=0)& (len(txtShotUtcDateTime.get())!=0)& (len(txtProfileID.get())!=0)& (len(txtComment.get())!=0):
            try:
                Eagle_SourceLink_Vibroseis_Log_BackEnd.addInvRec(txtTriggerIndex.get(), txtProfileID.get(), txtShotNumber.get(), txtEpNumber.get(), txtShotLine.get(), txtShotStation.get(),
                                                           txtShotUtcDateTime.get(), txtLatitude.get(), txtLongitude.get(), txtShotStatus.get(),
                                                           txtComment.get(), txtProcess.get(), txtSweepNumber.get())
                ...
                tkinter.messagebox.showinfo("Add Error","Invalid Data Type")
        else:
                tkinter.messagebox.showinfo("Add Error","Shot ID Line, Station, Shot time, ProfileID, Comment Can Not Be Empty")


    def VibroseisTB_MasterRec_Event(event):
        for nm in tree.selection():
            ...
            txtComment.insert(tk.END,sd[10])
            txtProcess.delete(0,END)
            txtProcess.insert(tk.END,sd[11])
            txtSweepNumber.delete(0,END)
            txtSweepNumber.insert(tk.END,sd[12])
    ...
    def iExit():
        iExit= tkinter.messagebox.askyesno("Eagle Clean Vib Master TB Import Wizard", "Confirm if you want to exit SourceLink Vib Master TB Widget")
        if iExit >0:
            window.destroy()
            return
    ...
    def UpdateMasterTB():
        conn = sqlite3.connect("SourceLink_Log.db")
        Complete_df_TB = pd.read_sql_query("SELECT * FROM Eagle_SOURCELINKTBMASTER ORDER BY `FileNum` ASC;", conn)
        ...
        data = data.loc[data.DuplicatedEntries == False, 'TriggerIndex': 'SweepNumber']
        data = data.reset_index(drop=True)
        data = pd.DataFrame(data)
        data.to_sql('Eagle_SOURCELINKTBMASTER',conn, if_exists="replace", index=False)
        conn.commit()
        conn.close()    
        TotalEntries()
    ...
    def ExportMasterTB_DB():
        conn = sqlite3.connect("SourceLink_Log.db")
        Complete_df = pd.read_sql_query("SELECT * FROM Eagle_SOURCELINKTBMASTER ORDER BY `FileNum` ASC ;", conn)
        ...
        Export_MasterTB_DF.rename(columns={'TriggerIndex':'TriggerIndex', 'Unit_ID':' ProfileId', 'FileNum':' ShotNumber',
                                                                ...
                                                                'ShotStatus':' ShotStatus', 'TBComment':' Comment', 'Process':' Process', 'SweepNumber': ' SweepNumber'},inplace = True)
    ...
    def ExportListBoxTB():
        ...
        ListBox_DF.rename(columns = {0:'TriggerIndex', 1:' ProfileId', 2:' ShotNumber', 3:' EpNumber', 4:' ShotLine',
                                      5: ' ShotStation', 6:' ShotUtcDateTime', 7:' Latitude', 8:' Longitude',9:' ShotStatus', 10:' Comment', 11:' Process', 12: ' SweepNumber'},inplace = True)                    
    ...
    def SortListBoxView():
        try:
            ...
            ListBox_DF.rename(columns = {0:'TriggerIndex', 1:'Unit_ID', 2:'FileNum', 3:'EPNumber', 4:'SourceLine',
                                         5: 'SourceStation', 6:'ShotUtcDateTime', 7:'Latitude', 8:'Longitude',9:'ShotStatus', 10:'TBComment', 11:'Process', 12: 'SweepNumber'},inplace = True)                        
    ...
    def ViewDuplicatedEntries():
        ...
        for i in range(len(List_DuplicatedFFID)):
            cur.execute("SELECT * FROM Eagle_SOURCELINKTBMASTER WHERE FileNum =?", (List_DuplicatedFFID[i],))
            rows=cur.fetchall()        
    ...
    def DeleteSelectedImportData():
        ...
        if iDelete >0:
            ...
            cur = conn.cursor()                
            for selected_item in tree.selection():
                cur.execute("DELETE FROM Eagle_SOURCELINKTBMASTER WHERE TriggerIndex =? AND FileNum =? AND EPNumber =? AND SourceLine =? AND \
                            SourceStation =? ",\
    ...
    def ImportTBFile():
        ...
        txtTotalEntries.delete(0,END)
        txtDuplicatedEntries.delete(0,END)
        fileList = askopenfilenames(initialdir = "/", title = "Import SourceLink Vib Clean Time Break Files" , filetypes=[('CSV File', '*.csv'), ('Excel File', ('*.xls', '*.xlsx'))])
        ...
        if Length_fileList >0:
            if fileList:
                ...
                for filename in fileList:
                    if filename.endswith('.csv'):
                        ...
                        try:
                            ...
                            Process       = df_TB.loc[:,' ShotStatus']
                        try:
                            SweepNumber   = df_TB.loc[:,' SweepNumber']
                        except:
                            SweepNumber   = df_TB.loc[:,' EpNumber']
                            
                        column_names = [TriggerIndex, Unit_ID, FileNum, EPNumber, SourceLine, SourceStation, ShotUtcDateTime,
                                        Latitude, Longitude, ShotStatus, TBComment, Process, SweepNumber]
                ...
                concatdf_TB.rename(columns={0:'TriggerIndex', 1:'Unit_ID', 2:'FileNum', 3:'EPNumber', 4:'SourceLine',
                                         ...
                                         9:'ShotStatus', 10:'TBComment' , 11:'Process', 12:'SweepNumber'},inplace = True)
                ...
                DATA_VALID_TB.to_sql('Eagle_SOURCELINKTBMASTER',con, if_exists="append", index=False)
            ...
            tkinter.messagebox.showinfo("Import Vib TB File Message","Please Select Vib TimeBreak Files To Import")
    ...
    txtTotalEntries.place(x=1150,y=6)
     ...
     ## Entry For Edit
    L3 = Label(TitFrame, text = "Edit Selected Master Database Vib Timebreak Entry : ", font=("arial", 10,'bold'),bg = "orange")
    L3.pack(side = TOP)
    ...
    txtTriggerIndex  = Entry(EditTableMargin, font=('aerial', 10, 'bold'),textvariable=StringVar(), width = 7)
    ...
    txtShotNumber  = Entry(EditTableMargin, font=('aerial', 10, 'bold'),textvariable=StringVar(), width = 20)
    ...
    txtEpNumber  = Entry(EditTableMargin, font=('aerial', 10, 'bold'),textvariable=StringVar(), width = 5)
    ...
    LabelLongitude = Label(EditTableMargin, text = "Lon", font=("arial", 10,'bold'))
    ...
    LabelComment.grid(row =2, column = 10 , sticky = W, padx= 0)
    txtComment  = Entry(EditTableMargin, font=('aerial', 10, 'bold'),textvariable=StringVar(), width = 12)
    txtComment.grid(row =4, column = 10 , sticky = W, padx= 0)

    LabelProcess = Label(EditTableMargin, text = "Process", font=("arial", 10,'bold'))
    LabelProcess.grid(row =2, column = 11 , sticky = W, padx= 0)
    txtProcess  = Entry(EditTableMargin, font=('aerial', 10, 'bold'),textvariable=StringVar(), width = 15)
    txtProcess.grid(row =4, column = 11 , sticky = W, padx= 0)

    LabelSweepNumber = Label(EditTableMargin, text = "Sweep#", font=("arial", 10,'bold'))
    LabelSweepNumber.grid(row =2, column = 12 , sticky = W, padx= 0)
    txtSweepNumber  = Entry(EditTableMargin, font=('aerial', 10, 'bold'),textvariable=StringVar(), width = 12)
    txtSweepNumber.grid(row =4, column = 12 , sticky = W, padx= 0)

    #----------------- Tree View Select Event------------
                
    tree.bind('<<TreeviewSelect>>',VibroseisTB_MasterRec_Event)

    ##### Labeling
    L1 = Label(window, text = " Vibroseis MasterDB Timebreak Details:", font=("arial", 10,'bold'),bg = "green").place(x=2,y=6)
    L2 = Label(window, text = "Duplicated Shot Line And Point", font=("arial", 10,'bold'),bg = "red").place(x=505,y=7)
    ...
    ### Button Bottom Wizard
    btnImportCleanTB = Button(TableMargin, text="Import Clean Vib TB File", font=('aerial', 9, 'bold'), height =1, width=20, bd=3, command = ImportTBFile)
    btnImportCleanTB.place(x=0,y=607)
    btnUpdateMasterDb = Button(TableMargin, text="Update MasterDB", font=('aerial', 9, 'bold'), height =1, width=16, bd=3, command = UpdateMasterTB)
    btnUpdateMasterDb.place(x=155,y=607)
    ...
    btnExportListBoxView = Button(TableMargin, text="Export List Box", font=('aerial', 9, 'bold'), height =1, width=13, bd=3, command = ExportListBoxTB)
    btnExportListBoxView.place(x=877,y=607)
    btnDelete = Button(TableMargin, text="Delete Selected", font=('aerial', 9, 'bold'), height =1, width=13, bd=3, command = DeleteSelectedImportData)
    btnDelete.place(x=980,y=607)
    btnClearView = Button(TableMargin, text="Clear View", font=('aerial', 9, 'bold'), height =1, width=10, bd=3, command = ClearView)
    btnClearView.place(x=1083,y=607)
    btnExit = Button(TableMargin, text="Exit Widget", font=('aerial', 9, 'bold'), height =1, width=10, bd=3, command = iExit)
    btnExit.place(x=1165,y=607)

    ## Bottom Last Buttons
    btnAddMasterDBEntry = Button(window, text="Add MasterDB Entry", font=('aerial', 10, 'bold'), height =1, width=16, bd=2, command = Add_Vib_MasterDB)
    btnAddMasterDBEntry.place(x=2,y=753)
    btnClearMasterDBEntry = Button(window, text="Clear Entry", font=('aerial', 10, 'bold'), height =1, width=10, bd=2, command = ClearEditEntry)
    btnClearMasterDBEntry.place(x=143,y=753)

    btnEditMasterDBEntry = Button(window, text="Edit MasterDB Entry", font=('aerial', 10, 'bold'), height =1, width=16, bd=2, command = Update_Vib_MasterDB)
