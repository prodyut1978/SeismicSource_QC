...
def SourceLink_TB_LogIMPORT():
    ...
    window = Tk()
    window.title ("Eagle SourceLink Vibroseis Timebreak Import Module")
    ...
    tree = ttk.Treeview(TableMargin, column=("column1", "column2", "column3", "column4", "column5",
                                             "column6", "column7", "column8", "column9", "column10", "column11"), height=26, show='headings')
    ...
    def ViewTotalImport():
        ...
        Complete_df_TB = pd.read_sql_query("SELECT * FROM Eagle_SOURCELINK_TB_TEMP ORDER BY `FileNum` ASC ;", conn)
    ...
    def ViewInvalidImport():
        ...
        txtDuplicatedEntries.delete(0,END)
        conn = sqlite3.connect("SourceLink_Log.db")
        Complete_df_TB = pd.read_sql_query("SELECT * FROM Eagle_SOURCELINK_TB_INVALID ORDER BY `FileNum` ASC ;", conn)
        data = pd.DataFrame(Complete_df_TB)
        data = data.reset_index(drop=True)
        for each_rec in range(len(data)):
            tree.insert("", tk.END, values=list(data.loc[each_rec]))
    ...
    def ViewDuplicatedImport():
        ...
        txtDuplicatedEntries.delete(0,END)
        conn = sqlite3.connect("SourceLink_Log.db")
        Complete_df_TB = pd.read_sql_query("SELECT * FROM Eagle_SOURCELINK_TB_Duplicated ORDER BY `FileNum` ASC ;", conn)
        data = pd.DataFrame(Complete_df_TB)
        data = data.reset_index(drop=True)
        for each_rec in range(len(data)):
            tree.insert("", tk.END, values=list(data.loc[each_rec]))
    ...
    def TotalEntries():
        conn = sqlite3.connect("SourceLink_Log.db")
        Complete_df_TB = pd.read_sql_query("SELECT * FROM Eagle_SOURCELINK_TB_TEMP ;", conn)
        data = pd.DataFrame(Complete_df_TB)
        TotalEntries = len(data)       
        txtTotalEntries.insert(tk.END,TotalEntries)              
        conn.commit()
    ...
    def InvalidEntries():
        conn = sqlite3.connect("SourceLink_Log.db")
        Complete_df_TB = pd.read_sql_query("SELECT * FROM Eagle_SOURCELINK_TB_INVALID ;", conn)
        data = pd.DataFrame(Complete_df_TB)
        TotalInvalidEntries = len(data)       
        txtInvalidEntries.insert(tk.END,TotalInvalidEntries)              
        conn.commit()
    ...
    def DuplicatedEntries():
        conn = sqlite3.connect("SourceLink_Log.db")
        Complete_df_TB = pd.read_sql_query("SELECT * FROM Eagle_SOURCELINK_TB_Duplicated ;", conn)
        data = pd.DataFrame(Complete_df_TB)
        TotalDuplicatedEntries = len(data)       
        txtDuplicatedEntries.insert(tk.END,TotalDuplicatedEntries)              
        conn.commit()
    ...
    def DeleteSelectedImportData():
        ...
        if iDelete >0:
            ...
            for selected_item in tree.selection():
                cur.execute("DELETE FROM Eagle_SOURCELINK_TB_TEMP WHERE TriggerIndex =? AND FileNum =? AND EPNumber =? AND SourceLine =? AND \
                            SourceStation =? ",\
    ...
    def ImportTBFile():
        ...
        if Length_fileList >0:
            if fileList:
                ...
                cur=con.cursor()                
                DATA_VALID_TB.to_sql('Eagle_SOURCELINK_TB_TEMP',con, if_exists="replace", index=False)
                Data_Invalid_TB.to_sql ('Eagle_SOURCELINK_TB_INVALID',con, if_exists="replace", index=False)
                DATA_VALID_TB_DuplicatedShotID.to_sql ('Eagle_SOURCELINK_TB_Duplicated',con, if_exists="replace", index=False)                
                con.commit()
                cur.close()
                con.close()
                ViewTotalImport()
        else:
            tkinter.messagebox.showinfo("Import Vib TB File Message","Please Select Vibroseis TimeBreak Files To Import")
    ...
    L1 = Label(window, text = "Source Link Vibroseis Timebreak Details:", font=("arial", 10,'bold'),bg = "green").place(x=2,y=6)
    L2 = Label(window, text = "Null ShotID Or File Number", font=("arial", 10,'bold'),bg = "red").place(x=505,y=7)
    L3 = Label(window, text = "Duplicated ShotID Or File Number", font=("arial", 10,'bold'),bg = "red").place(x=505,y=621)
    ...
    btnImportTBFile = Button(window, text="Import Vib TB File", font=('aerial', 9, 'bold'), height =1, width=15, bd=2, command = ImportTBFile)
    btnImportTBFile.place(x=2,y=620)

    btnDuplicatedImport= Button(window, text="View Duplicated", font=('aerial', 9, 'bold'), height =1, width=14, bd=2, command = ViewDuplicatedImport)
    btnDuplicatedImport.place(x=314,y=620)
    btnDelete = Button(window, text="Delete Selected Valid", font=('aerial', 9, 'bold'), height =1, width=18, bd=2, command = DeleteSelectedImportData)
    btnDelete.place(x=930,y=620)
    btnClearView = Button(window, text="Clear View", font=('aerial', 9, 'bold'), height =1, width=10, bd=2, command = ClearView)
    btnClearView.place(x=1085,y=620)
    btnExit = Button(window, text="Exit Import", font=('aerial', 9, 'bold'), height =1, width=10, bd=2, command = iExit)
    btnExit.place(x=1168,y=620)
